...
class WifiDirectManager(
    ...
    override fun onReceive(context: Context, intent: Intent) {
        when (intent.action) {
            WifiP2pManager.WIFI_P2P_STATE_CHANGED_ACTION -> {
                ...
                Log.e(
                    "WFDManager", "The WiFi direct adapter state has changed to $isWifiP2pEnabled"
                )
            }
    ...
    fun connectToPeer(peer: WifiP2pDevice) {
        ...
        manager.connect(channel, config, object : ActionListener {
            override fun onSuccess() {
                Log.e(
                    "WFDManager", "Successfully attempted to connect to a peer '${peer.deviceName}'"
                )
            }
            ...
            override fun onFailure(reason: Int) {
                Log.e(
                    "WFDManager",
                    "An error occurred while trying to connect to a peer '${peer.deviceName}'"
                )
            }
